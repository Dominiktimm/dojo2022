...
let game_over = false;
// User Input - which light do you want to toggle
    // Input into coordinates
    // Check for neighbors 
    // Switch input and neighbors status
let game = {
    selection:[],
    ...
    input() {
        ...
        if(input_field <= 9 && input_field > 0) {
            this.input_to_coordinates(input_field);
        } else {
            if (confirm("To continue press OK")) {
                game.start();
              } else {
                game_over = true;
              }
